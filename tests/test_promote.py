...
def test_vault_promotion(registry, vault, rando, gov):
    ...
    with brownie.reverts():
        registry.promote(vault, "v1", "DCA-BTC-CVX", 1, {"from": rando})

    # Governance is able to promote vault
    tx = registry.promote(vault, "v1", "DCA-BTC-CVX", 1, {"from": gov})
    ...
    tx = registry.promote(vault, "v1", "DCA-BTC-CVX", 2, {"from": gov})
...
def test_vault_promotion_by_strategist_guild(registry, vault, strategistGuild):
    registry.promote(vault, "v1", "DCA-BTC-CVX", 1, {"from": strategistGuild})
    assert registry.getFilteredProductionVaults("v1", 1) == [[vault, "v1", "1", "DCA-BTC-CVX"]]
...
def test_vault_promotion_step_prod(registry, vault, rando, gov):
    ...
    registry.promote(vault, "v1", "DCA-BTC-CVX", 3, {"from": gov})
    assert registry.getFilteredProductionVaults("v1", 3) == [[vault, "v1", "3", "DCA-BTC-CVX"]]
...
def test_vault_promotion_step_deprecated(registry, vault, rando, gov):
    registry.promote(vault, "v1", "DCA-BTC-CVX", 0, {"from": gov})
...
def test_vault_promotion_permissions(registry, vault, rando, gov, devGov):
  ...
  with brownie.reverts():
    registry.promote(vault, "v1", "DCA-BTC-CVX", 2, {"from": rando})
  
  ## Even though we put 2 here, we still only go to 0 because devGov is limited to it
  registry.promote(vault, "v1", "DCA-BTC-CVX", 2, {"from": devGov})
  assert registry.getFilteredProductionVaults("v1", 1) == [[vault, "v1", "1", "DCA-BTC-CVX"]]
